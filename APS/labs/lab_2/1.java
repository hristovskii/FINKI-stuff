...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        ...
        double min = average - arr.get(0).length();
        int position = 0;
        for (int i = 0; i < arr.getSize(); i++) {
            if (Math.abs(arr.get(i).length() - average) < min){
                position = i;
                min = Math.abs(arr.get(i).length() - average);
            }
            if(Math.abs(arr.get(i).length() - average) == min){
                if (arr.get(position).length() < arr.get(i).length()){
                    position = i;
                    min = Math.abs(arr.get(i).length() - average);
                }
            }
