...
Input	     Result
84 44 87     84 79 26 57 45 61 4  
79 9 67      10 13 64 70 55 93 60 14 50
26 2 81
57 91 59
45 92 8
61 52 95
...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        SLLNode<Card> previousNode;
        int size = secondSorcererCards.size();
        int i = 0;
        previousNode = secondSorcererCards.getFirst();
        while (i<(size/2 - 1)){
            previousNode = previousNode.succ;
            i++;
        }

        SLLNode<Card> card = firstSorcererCards.getFirst();
        double max = 0;
        SLLNode<Card> node = firstSorcererCards.getFirst();
        for (int j = 0; j < firstSorcererCards.size(); j++) {
            if (node.element.significance()> max){
                max = node.element.significance();
                card = node;
            }
            node = node.succ;
        }
        firstSorcererCards.delete(card);

        secondSorcererCards.insertAfter(card.element, previousNode);
    }
